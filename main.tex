...
\ss{5.5}
\ms I think this is right, but haven't 100\% thought it through. $x$ and $y$ are as in the problem, $y'$ and $r$ are initially 0, and the answer should be in $r$.
...
\path[->] (subyp) edge [bend left=90] node {$e$} (xtor);
\end{tikzpicture}
\end{center}
