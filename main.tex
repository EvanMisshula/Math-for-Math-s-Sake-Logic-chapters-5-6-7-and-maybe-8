...
\node (end1) [below right=of addm] {};
\node (end2) [below=of subn] {};
\path[->] (start) edge (subn);
\path[->] (subn) edge node {$e$} (addm);
\path[->] (addm) edge (end1);
\path[->] (subn) edge (end2);
\end{tikzpicture}
\end{center}
\ss{5.3}
\ms Since we know that exponentiation is abacus computable, and we know the zero function is abacus computable, the composition $f(x)=0^x$ is abacus computable.  But this is equal to
$\mathrm{sg}(x)$, since $0^0 = 1$ but $0^x = 0$ for $x>0$.
\ss{5.4}
\ss{5.5}
\ss{5.6}
\ss{5.7}
...
last chapter.  $\mathrm{lg}(x, y)$ is the minimum $z$ for which
this is true.  In general, minimization isn't a primitive recursive operation, however we can put a bound on $z$: it can be no bigger than $x$, for example.  Thus we can use bounded minimization, which by Corollary 7.6, gives a primitive recursive function.
\ss{7.3}
\ss{7.5}
\ss{7.12}
\ss{7.13}
